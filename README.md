...
- 🔐 **多账号管理** — 支持同时管理抖音、小红书、快手、微信视频号等平台的多个账号，独立 session，互不影响
...
| **快手自动发布技术文档**   | [`flowx-desktop/docs/快手自动发布技术文档.md`](./flowx-desktop/docs/快手自动发布技术文档.md)     | Element UI / contenteditable / user-cnt\_\_item 粉丝数解析                                          |
...
### 🍎 macOS 运行提示 "FlowX已损坏，无法打开。你应该将它移到废纸篓"

由于测试/内测版本未配置苹果官方开发者证书进行签名，打包应用在 macOS 上运行会被系统的 Gatekeeper 机制拦截。特别是通过企业微信、浏览器下载的应用文件，会被自动标记为 `quarantine` (隔离) 状态。

**解决办法：**

#### 方式一：一键绕过当前应用隔离限制（最直接，推荐）

1. 将挂载的 DMG 安装包内的 `FlowX` 图标拖拽到 **`Applications`（应用程序）** 文件夹中。
2. 打开 Mac 的 **终端（Terminal）** 窗口。
3. 复制并执行以下命令解除隔离属性：
   ```bash
   sudo xattr -r -d com.apple.quarantine /Applications/FlowX.app
   ```
4. 输入您的 Mac 开机密码并按回车确认。之后在应用程序中双击即可直接打开。

#### 方式二：全局启用系统「任何来源」安全信任（一劳永逸）

1. 打开 Mac 的 **终端（Terminal）** 窗口。
2. 输入以下命令并回车，以强制开启 macOS 隐藏的「任何来源」选项：
   ```bash
   sudo spctl --master-disable
   ```
3. 输入开机密码确认。
4. 打开系统的 **「系统设置」 -> 「隐私与安全」**，向下滚动到「安全性」区域，将“允许以下来源的应用程序”勾选为 **「任何来源」** 即可。

_(注：如果不想使用终端，也可以在双击拦截后，立即进入「系统设置」 -> 「隐私与安全」，在被阻止提示的旁边点击「仍要打开」按钮进行单次授权。)_
...
- **v1.1** — 发布历史页 / 数据统计 / 更稳定的图文发布
- **v1.2** — 支持更多平台（B 站、微信公众号、知乎等）
