...
- 🔐 **多账号管理** — 支持同时管理抖音、小红书、快手、微信视频号、微信公众号、知乎、今日头条、X（Twitter）、B 站（哔哩哔哩）、微博等平台的多个账号，独立 session，互不影响
...
- 🎯 **自定义站点规则** — 可视化拾取网页元素，自定义内容提取规则，支持域名/正则匹配，优先级高于内置规则
- 🧩 **浏览器内规则管理** — 右侧面板 Tab 式管理（发布编辑 + 提取规则），边浏览边配置，所见即所得
- 🏷️ **内容类型匹配** — 规则支持图文/视频/文章多类型匹配，自动按发布类型选择最优规则
- 📊 **账号数据分析** — 多平台作品数据采集（抖音/小红书/快手/视频号），支持按账号/平台/关键词筛选，真分页，数据本地存储，隐私安全
...
> **仅账号管理平台**：微信公众号、今日头条、X（Twitter）、B 站（哔哩哔哩）、微博目前仅支持账号管理（登录态检测、账号信息提取、打开创作中心），发布功能待开发。微信公众号因需要管理员扫码确认发布，暂不支持自动发布。知乎已支持视频发布和图文发布功能。X 平台优先从 `window.__INITIAL_STATE__` 提取昵称/头像/handle，同时支持 `SideNav_AccountSwitcher_Button`、`UserAvatar-Container-<handle>` 等多数据源兜底。B站使用 SESSDATA 核心 cookie + DedeUserID 读取 UID，并与微信视频号共享相同级别的 28 种字符黑名单严格净化。微博使用 SUB cookie 核心登录态，支持 me.weibo.com 创作中心提取微博号/UID 及粉丝数据。
...
| **微博平台接入技术文档** | [`flowx-desktop/docs/微博平台接入技术文档.md`](./flowx-desktop/docs/微博平台接入技术文档.md) | 账号管理：SUB Cookie 登录态 / me.weibo.com 创作中心 / 微博号或UID双ID支持 / sinaimg.cn 强制HTTPS |
| **平台适配器目录** | `flowx-desktop/src/main/services/platforms/` | xiaohongshu.ts / douyin.ts / kuaishou.ts / zhihu.ts（独立实现 + 共享 shared.ts） |
...
- **v1.2 ✅（已实现）** — 账号管理平台扩展（微信公众号 ✅、知乎 ✅、今日头条 ✅、X（Twitter） ✅、B 站（哔哩哔哩）✅、微博 ✅）/ 用户自定义站点规则 / 可视化元素拾取 / 浏览器内规则管理面板
- **v1.3 ✅（已实现）** — 账号数据分析（作品采集/多条件筛选/真分页），微信视频号图文采集，授权对话框平台两列栅格布局优化
- **v1.4** — 插件系统（允许用户编写自定义平台适配器）
