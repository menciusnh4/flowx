...
> **文章发布**：支持抖音和小红书的长文创作。抖音文章标题 30 字、正文 8000 字；小红书文章标题 64 字、正文不限制。小红书文章需走"一键排版 → 下一步 → 发布"的三步点击流程。

> 各平台图文/文章发布的技术细节：CDP 文件上传 → ProseMirror 富文本填充 → Shadow DOM 按钮点击。详见 `docs/` 目录下的平台技术文档。
