...
> **图文发布**：支持封面图上传 + 标题/正文/话题填写，每个平台独立的 DOM 选择器和事件模拟。微博图文发布在首页 `weibo.com` 顶部发布卡片，标题与正文合入同一个 textarea（标题换行后是正文），图片上传注入真实隐藏 `input._file_hqmwy_20` 后校验缩略图+删除按钮。
...
- **v1.4 ✅（已实现）** — 微博视频发布（upload/channel + 封面生成等待）+ 微博图文/纯文本发布（首页发布卡片 + 图片缩略图校验），相关配套技术文档
- **v1.4.1 ✅（已实现）** — 今日头条微头条+西瓜视频发布 / B站账号管理 / 账号头像本地持久化 / 系统托盘 / macOS适配 / 8平台文档体系重构 → v0.1.6
- **v1.5** — 插件系统（允许用户编写自定义平台适配器）
