...
- 🔐 **多账号管理** — 支持同时管理抖音、小红书、快手等平台的多个账号，独立 session，互不影响
...
⚠️ = 平台适配器已预留接口，具体 DOM 适配需按平台更新持续完善
...
1. **平台适配器模式** — 每个平台的发布流程抽象为 `meta`（URL/选择器）+ `detectLoggedIn` + `publishKeywords`，90% 的流程由通用的 `runStandardPublish` 处理
2. **导航稳定性** — 页面跳转后通过 `NavigationTracker` 等待 1.5~3 秒静默期，再执行 DOM 操作，避免 "Render frame was disposed"
3. **账号隔离** — 每个账号独立 `persist:account_{id}` partition，天然隔离登录态
4. **Electron 版本策略** — 固定在 31.7.7（Chromium 126），已验证无渲染进程崩溃
5. **React 受控组件兼容** — 表单填写通过原生 setter + `input/change/blur` 三重事件派发，确保 React state 同步
