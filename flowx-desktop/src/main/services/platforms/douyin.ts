...
// === 内容填写脚本（生成 JS 字符串，避免模板字符串嵌套问题）
function buildFillTitleScript(text: string): string {
  const textJson = JSON.stringify(text);
  return `(function () { var text = ${textJson}; if (!text) return { ok: true, skipped: true }; var found = false; try { var n1 = document.querySelectorAll('[contenteditable]'); for (var i1 = 0; i1 < n1.length; i1++) { var e1 = n1[i1]; var ceVal1 = e1.getAttribute && e1.getAttribute('contenteditable'); if (ceVal1 === 'false') continue; var cls1 = String(e1.className || ''); if (/tiptap|ProseMirror|prosemirror/i.test(cls1)) continue; e1.focus(); try { document.execCommand('selectAll'); document.execCommand('delete'); } catch (eX1) {} document.execCommand('insertText', false, text); return { ok: true, method: 'contenteditable' }; } } catch (e1) {} try { var ta1 = document.querySelectorAll('textarea, input[type="text"]'); for (var j1 = 0; j1 < ta1.length; j1++) { var e2 = ta1[j1]; e2.focus(); e2.value = text; try { var ev1 = document.createEvent('Event'); ev1.initEvent('input', true, true); e2.dispatchEvent(ev1); } catch (eErr1) {} return { ok: true, method: 'textarea' }; } } catch (e2) {} return { ok: false, reason: 'no-title-field' }; })();`;
}

function buildFillContentScript(content: string): string {
  const contentJson = JSON.stringify(content);
  return `(function () { var content = ${contentJson}; if (!content) return { ok: true, skipped: true }; try { var n1 = document.querySelectorAll('[contenteditable]'); for (var i1 = 0; i1 < n1.length; i1++) { var e1 = n1[i1]; var ceVal = e1.getAttribute && e1.getAttribute('contenteditable'); if (ceVal === 'false') continue; var cls1 = String(e1.className || ''); if (/tiptap|ProseMirror|prosemirror/i.test(cls1)) { e1.focus(); try { document.execCommand('selectAll'); document.execCommand('delete'); } catch (eX1) {} var lines = content.split('\\n'); for (var li = 0; li < lines.length; li++) { document.execCommand('insertText', false, lines[li]); if (li < lines.length - 1) document.execCommand('insertText', false, '\\n'); } return { ok: true, method: 'prosemirror' }; } } } catch (e1) {} try { var n2 = document.querySelectorAll('[contenteditable]'); for (var k1 = 0; k1 < n2.length; k1++) { var e2 = n2[k1]; var ceVal2 = e2.getAttribute && e2.getAttribute('contenteditable'); if (ceVal2 === 'false') continue; e2.focus(); try { document.execCommand('selectAll'); document.execCommand('delete'); } catch (eX2) {} document.execCommand('insertText', false, content); return { ok: true, method: 'plain-ce' }; } } catch (e2) {} try { var ta1 = document.querySelectorAll('textarea'); for (var j1 = 0; j1 < ta1.length; j1++) { var e3 = ta1[j1]; e3.focus(); e3.value = content; try { var ev2 = document.createEvent('Event'); ev2.initEvent('input', true, true); e3.dispatchEvent(ev2); } catch (eErr2) {} return { ok: true, method: 'textarea' }; } } catch (e3) {} return { ok: false, reason: 'no-content-field' }; })();`;
}

function buildClickPublishScript(): string {
  return `(function () { var candidates = []; try { var all = document.querySelectorAll('button, div, a, span'); for (var i = 0; i < all.length; i++) { var el = all[i]; var txt = (el.innerText || el.textContent || '').trim(); if (!txt) continue; if (/发布管理|发布作品管理|作品管理|发布列表/.test(txt)) continue; if (txt.indexOf('发布') === -1 && txt.indexOf('立即') === -1) continue; var cls = String(el.className || ''); var score = 0; if (/发布作品|立即发布|确认发布|发布视频|发布文章/.test(txt)) score += 1500; else if (/发布|发布图文|发布/.test(txt)) score += 1000; if (/publish|ce-btn|primary-button|publish-btn|btn-publish|publish-button|button-primary/i.test(cls)) score += 800; try { if ((el.offsetWidth || 0) < 10 || (el.offsetHeight || 0) < 10) continue; var st = window.getComputedStyle(el); if (st && (st.display === 'none' || st.visibility === 'hidden' || parseFloat(st.opacity || '1') < 0.3)) continue; } catch (eSz) {} candidates.push({ el: el, score: score, text: txt.slice(0, 40), cls: cls.slice(0, 80) }); } } catch (e) {} if (candidates.length > 0) { candidates.sort(function (a, b) { return b.score - a.score; }); var top = candidates[0]; try { top.el.click(); return { clicked: true, text: top.text, score: top.score, strategy: 'js-click' }; } catch (e) { try { var ev = document.createEvent('Event'); ev.initEvent('click', true, true); top.el.dispatchEvent(ev); return { clicked: true, text: top.text, score: top.score, strategy: 'event' }; } catch (e2) { return { clicked: false, reason: 'click-failed', text: top.text }; } } } return { clicked: false, reason: 'no-match' }; })();`;
}

function buildPublishResultProbeScript(): string {
  return `(function () { var url = location.href; var bodyText = document.body ? (document.body.innerText || '') : ''; var title = document.title || ''; var successText = bodyText + ' | ' + title; var success = /发布成功|发表成功|已发布|发布完成|作品已发布|投稿成功/i.test(successText); var fail = /发布失败|不符合|违规|失败|无法发布|异常|错误/i.test(successText); var leftPage = !/upload|publish|create|creator|content\\/publish|content\\/upload|creator-micro/i.test(url); return { url: url, text: successText.slice(0, 400), success: success || leftPage, fail: fail }; })();`;
}
...
async function runDouyinPublish(accountId: string, request: PublishRequest, onProgress: ProgressCallback, contentType: ContentType): Promise<PublishItemProgress> {
  ...
  try {
    ...
    const mediaType = contentType === 'image' ? '图片' : (contentType === 'article' ? '图文素材' : '视频');
    ...
    const resultDeadline = Date.now() + 90000;
